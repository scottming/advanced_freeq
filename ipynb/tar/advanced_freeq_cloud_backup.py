...
Usage:
    ...
    ./advanced_freeq -p <pdfname>  [-o <output>] [-c] [--mas=<mastered> --mas=<mastered>] [-w <wordimg>]
...
Options:
    ...
    -w --wordcloud      Show and Save wordcloud image
...
if arguments['--coca'] == True:
    ...
    if arguments['--wordcloud'] == True:
        df_freq = df_freq[['Word','Freq']]
        tuples = [tuple(x) for x in df_freq.values]
        ...
        plt.axis("off")
        plt.savefig('%s' % arguments['<wordimg>'], dpi=300)
        plt.show();
